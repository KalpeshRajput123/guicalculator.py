...
first = second_number = operator = None
...
def calculate():
    global first, second_number, operator
    second_number = int(result_label['text'])
    if operator == "+":
        result_label.config(text=str(first + second_number))
    elif operator == "-":
        result_label.config(text=str(first - second_number))
    elif operator == "*":
        result_label.config(text=str(first * second_number))
...
btn_equals = Button(root, text='=', bg="#00a65a", fg='white', width=5, height=2, command=calculate)
